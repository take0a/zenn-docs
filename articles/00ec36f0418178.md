...
既に、AOSS の検索コレクションを使っていて、開発環境だから OCU は最小の 2 に設定している場合、ベクトル検索コレクションを追加すると 4 に上げないとコレクションが作れません。また、AOSS を使っていない場合、だいぶ安くなったが、それなりに費用がかかるので、事前に確認することをお勧めする。
