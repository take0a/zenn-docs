...
## MCPサーバの実行
テスト環境は、少し古い本ですが、「[業務知識と情報システム](https://www.amazon.co.jp/dp/4876014272)」という書籍に記載されている製造業をモデルに、ほぼ全ての業務が使用するファイルとその項目の情報を PostgreSQL 上のテーブルとして登録しました。

原本では、これらのファイルから出力することになっている帳票を作るために何をすれば良いのか？を AI エージェントに考えてもらうシナリオです。チャットの指示は、以下の流れで行いますが、AI エージェントが自分で次の指示まで行う場合は、暖かく見守ります。また、うまくできない場合は、指示を微修正しました。
