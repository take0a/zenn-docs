...
これらのカラムの値をSQLを介して、データベースと入出力するのであるが、`database/sql`の場合、上のように、`Exec`の引数も`Scan`の引数も項目毎に指定する必要がある。こういう場合もあるのだろうから、こういう指定もできるようにすべきなのだろうだが、構造体をマルっと指定できるようにもできないものだろうか。
...
```diff go: sql/main.go sqlx1/main.go
4d3
<       "database/sql"
7a7
>       "github.com/jmoiron/sqlx"
...
さて、最初に提起した課題は、以下の３点でした。
- Exec で入力として構造体を指定したい
- Scan で出力として構造体を指定したい
- プレースホルダーの方言を意識したくない
...
先程との違いは、`NamedExec` `StructScan` に加えて、構造体に `db`タグを使用している箇所とSQL文中のプレースホルダーである。

```diff go: sqlx1/main.go sqlx2/main.go
13,15c13,15
<       CustomerID int
<       Name       string
<       Address    string
...
```diff go: sqlx2/main.go sqlx3/main.go
45c45
<       err = db.QueryRowx(`
---
>       query, args, err := db.BindNamed(`
...
sqlx は、非常に簡潔に問題を解決してくれました。また、sqlx 自体も非常にコンパクトに書かれていて、中身を読むことで勉強になりました。
今回のソースコードは、上に貼ったとおりですが、github にも残しておきます。
