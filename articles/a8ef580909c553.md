...
[オリジナルのテンプレート](https://github.com/microsoft/vscode-python-tools-extension-template)を fork して、ローカルに clone して、`README.md` に従って進めます。
...
- `<pytool-module>` - `pyext`
- `<pytool-display-name>` - `VSCode Python Extension`
- `<my-publisher>` - `robon`
- `<my-github>` - `take0a`
- `<my-extension-project>` - `vscode-python-tools-extension-template`
- `<server version>` - `2022.0.0-dev`
...
`pygls.server.LanguageServer` が実体です。LSP の feature を登録するデコレータで関数を紐づけるので、ここでは、`@LSP_SERVER.feature()` デコレータが使われています。
